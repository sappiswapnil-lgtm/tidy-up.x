-- ============================================================================
-- BidBlock — COMBINED REPAIR MIGRATION (v2)
-- ============================================================================
-- Run this ENTIRE file once, top to bottom, in the Lovable Cloud / Supabase
-- SQL editor. It is fully idempotent: safe to re-run, safe on a database where
-- an earlier attempt failed halfway through.
--
-- WHY YOUR APP IS THROWING 400s AND 404s
--   400 on  /rest/v1/auctions?...visibility=eq.public   -> column "visibility" does not exist
--   400 on  POST /rest/v1/auctions                      -> insert references columns that don't exist
--   404 on  /rest/v1/auction_participants               -> that table does not exist
-- The database only has migrations 0000-0002. Migrations 0003 and 0004 were
-- never applied (they are missing from drizzle/migrations/meta/_journal.json,
-- so the migration runner never picked them up). This file applies them.
--
-- WHAT WAS BROKEN IN THE PREVIOUS VERSION OF THIS FILE (now fixed)
--   1. Infinite RLS recursion: the auctions policy queried auction_participants
--      while the auction_participants policy queried auctions. Postgres raises
--      "infinite recursion detected in policy for relation ..." (error 42P17) on
--      every read. Both sides now go through SECURITY DEFINER helper functions.
--   2. Ambiguous column: "WHERE p.auction_id = id" inside the auctions policy
--      resolved `id` to auction_participants.id (innermost scope wins), not
--      auctions.id — so participants could never see a private auction. Now
--      fully qualified.
--   3. Invited-but-not-yet-joined users could not see the auction row at all,
--      so the "Invites you've received" list rendered empty. Viewing now
--      requires only an invite; bidding still requires joining.
--   4. ALTER PUBLICATION could fail when supabase_realtime doesn't exist.
--      Now guarded.
--   5. GET DIAGNOSTICS wrote ROW_COUNT (integer) into a boolean variable.
--      Now uses an integer and compares.
--   6. The old 2-argument place_bid could linger and make calls ambiguous.
--      Both old signatures are dropped explicitly.
-- ============================================================================
...
-- ============================================================================
-- PART 5 — tell PostgREST to reload, then verify
-- ============================================================================
...
DECLARE
  ...
  c text;
BEGIN
  FOREACH c IN ARRAY ARRAY['visibility','participant_limit','item_count','locked','starts_at',
                           'listing_currency','payment_confirmed_at','payment_confirmed_by','leader_name'] LOOP
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'auctions' AND column_name = c
    ) THEN
      v_missing := v_missing || c;
    END IF;
  END LOOP;

  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='auction_participants') THEN
    v_missing := v_missing || 'table:auction_participants';
  END IF;

  IF array_length(v_missing, 1) IS NULL THEN
    RAISE NOTICE 'OK — schema is up to date. Hard-refresh the app (Ctrl/Cmd+Shift+R).';
  ELSE
    RAISE EXCEPTION 'Migration incomplete, still missing: %', array_to_string(v_missing, ', ');
  END IF;
END $$;

-- Final sanity check — should return one row listing the new columns.
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_schema = 'public' AND table_name = 'auctions'
  AND column_name IN ('visibility','item_count','locked','starts_at','listing_currency','leader_name')
